...
def __init__(self):
    self.filter_exclusion = r'.*@example\.com|John Smith|John Doe|Jane Smith|Jane Doe'
...
def generate(self, input_list, _id):
    ...
    for input_string in input_list:
        try:
            ...
            self.extract_field(obj, item, "Court Name")
            self.extract_field(obj, item, "Petition Type")
            self.extract_field(obj, item, "Order Date")
            self.extract_field(obj, item, "Case Number")
            self.extract_field(obj, item, "Case Date")
            for field in self.field_list:
...
def extract_field(self, obj, item, field_name):
    if field_name in item and item[field_name]:
        ...
        if isinstance(val_object, str) and self._is_valid_value(val_object):
            if field_name.lower().endswith("date"):
                formatted_date = self._get_date_value(val_object)
                if formatted_date:
...
def _is_valid_value(self, input_string):
    if not self.filter_exclusion:
        return True
    regex = re.compile(self.filter_exclusion)
